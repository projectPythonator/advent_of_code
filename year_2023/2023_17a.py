...
class SSSP_Solver():
    ...
    def dijkstras(self, s, goal):
        ...
        while q:
            ...
            for v in self.dir2walks[d]:
                r = i + self.drc[v][0]
                c = j + self.drc[v][1]
                n_l = 2 if v!=d else l-1
                if (n_l < 0) or (r < 0 or c < 0 or r >= len(self.grid) or c >= len(self.grid[0])):
                    continue
    ...
    def solve_SSSP(self):
        print(self.dijkstras((0, 0, 0, 3, 1), (len(self.grid)-1, len(self.grid[0])-1)))
